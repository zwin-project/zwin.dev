...
## 注意
Beta版のzwinはまだ不完全で、このドキュメントでは以下の注意点があります。

- Wayland Client以外の2Dアプリケーションが動きません。
  - 特にVSCodeなどのアプリが動きません。
  - 代わりに、**VimやnanoなどCUIのエディタを使ってください**。

- Wayland Clientの一部の機能が動きません。
...
zwinでは、あなたは空間全体を占有する"Big App"を作る代わりに、空間の一部を占有する**小さな**アプリケーションを作ることができます。小さいので、少数の機能だけにフォーカスできます。

zwinではOpenGL ESに似た比較的低いレイヤのプロトコルを通して3Dアプリケーションを作ることができます。アプリの描画についての詳細は、[レンダリング方式](/what_is_it/rendering_scheme)を参照してください。

ここでは、簡単なクライアントライブラリである[zukou](https://github.com/zwin-project/zukou)を使ったアプリケーションを作ります。

## ドキュメントをブラウザで開く
zen-desktopはデスクトップ環境なので、ブラウザを開くことができます。
例えば[Google Chrome](https://www.google.com/chrome/)をインストールしている方は、以下のように、LauncherのアイコンとしてChromeを追加してみてください。
...
LauncherのGoogle Chromeのアイコンをクリックすればブラウザが開きます。

![2d launcherでこのページを開くGIF](image1.png "image_tooltip")

ブラウザでこのドキュメントを開けば、VR空間から抜けずに、ドキュメントを見ながら作業できます！

![3d launcherでこのページを開くGIF](image1.png "image_tooltip")
...
.
```
そしてzen-desktopをログアウトして再起動すれば、ターミナルのアイコンが現れます。
これをクリックして、ターミナルを開きます。

![launcherでターミナルを開くGIF](image1.png "image_tooltip")

ターミナルを開いたら、コマンドを入力していきます。
Beta版のzwinでは、 以下の注意点があります。
- VSCodeなどの、Wayland Clientではない2Dアプリはまだサポートしていないので、**VimやnanoなどのCUIエディタを利用してください。**
...
`Ctrl + C`か`pkill celestial-body-app`などでAppを閉じます。
...
## 複雑なロジックの追加
前の項目では、設定ファイルでも変更できそうな、簡単なパラメータの変更のみとなりましたが、アプリケーションのソースコードはC++であり、もちろん複雑なロジックも追加することができます。

zwinでアプリケーションを作る上での制約となるのは、OpenGL ESライクな描画と入力に関するプロトコルです。
プロトコルとクライアントの関係は[Zwinとは？](ja/what_is_it/what_is_zwin)のドキュメントをご覧ください。
プロトコルの実際の中身は[こちら](https://github.com/zwin-project/zwin/tree/main/protocol)をご覧ください。

地球をRayのスクロールで回転できるようにしましょう。
まず、アプリケーションが入力を受け取れるようにします。
そのためには、Regionという入力領域を3DアプリにSetします。コード上では次のような変更をします。
```:src/main.cc
.
.
.
...
Regionをつけたことで、その領域とRayが交差してスクロールのイベントが発生した時に`RayAxisFrame`が呼ばれます。これはzukouというライブラリによる抽象化で、実際にはプロトコルから生成されたヘッダファイルを使って振る舞いを記述できます。プロトコルは特定の言語に依存していないので、別の言語で3Dアプリを実装することも可能です。
```
.
.
  // 関数の中身を実装
...
この値からラジアンの回転角`theta`を求め、惑星`sphere_`を回転させる行列を算出し、`sphere_`の位置を保持する行列を更新しています。
...
これで地球儀に近いアプリケーションができました！
zwinでは、このようにアプリケーションの修正とビルドと実行をVR空間の中で全て行うことができます。

## まとめ

上記で実装したように、zwinでは特定の空間を占有する**小さな**アプリケーションを実装することができます。

開発者の目線でのメリットは以下です。
- Appを修正、再ビルド、実行を全てVR空間の中で完結して行える
- 小さいAppを作れる
  - "Big App"のように"全て"を作る必要はないのです！
  - 今のVRは例えば時計を表示するアプリを作るために部屋も作らなければいけません。時計だけ作りたいですよね？zwinならできます。

ユーザーもこの小さいAppを組み合わせて使うことができます。
zwinは生まれたばかりのプロジェクトなので、

UNIX哲学の[Do one thing, and do it well.](https://en.wikipedia.org/wiki/Unix_philosophy#Do_One_Thing_and_Do_It_Well)を体現するアプリケーションで、自由でオープンなVRやAR空間を築きましょう。
